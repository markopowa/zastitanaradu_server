...
def clear_jwt_cookies(response: HttpResponse) -> None:
    access_name, refresh_name = get_jwt_cookie_names()
    kwargs = _cookie_kwargs()
    response.delete_cookie(access_name, path=kwargs["path"], samesite=kwargs["samesite"])
    response.delete_cookie(refresh_name, path=kwargs["path"], samesite=kwargs["samesite"])
