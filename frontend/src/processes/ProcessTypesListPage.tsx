  ...
  doDelete = (): void => {
    ...
    deleteProcessType(deleteConfirmId)
      ...
      .catch((err: { response?: { data?: { detail?: string } } }) => {
        const msg =
          err.response?.data?.detail ?? "Greška pri brisanju vrste obaveze.";
  ...
  render(): React.ReactNode {
    ...
    return (
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        {loading ? (
          ...
          <ScrollableTablePaper>
            <Table size="small" sx={{ width: "100%", tableLayout: "fixed" }}>
              <TableHead>
                <TableRow>
                  <TableCell sx={tableCellEllipsis}>Šifra</TableCell>
                  <TableCell sx={tableCellEllipsis}>Naziv</TableCell>
                  <TableCell sx={tableCellEllipsis}>Subjekt</TableCell>
                  <TableCell sx={tableCellEllipsis}>Period (meseci)</TableCell>
                  <TableCell sx={tableCellEllipsis}>Rok isporuke (dana)</TableCell>
              ...
              <TableBody>
                {items.map((row) => (
                  <TableRow key={row.id}>
                    <TableCell sx={tableCellEllipsis}>{row.code}</TableCell>
                    <TableCell sx={tableCellEllipsis}>{row.name}</TableCell>
                    <TableCell sx={tableCellEllipsis}>
                      {SUBJECT_OPTIONS.find((s) => s.value === row.subject_kind)
                        ?.label ?? row.subject_kind}
                    </TableCell>
                    <TableCell>{row.default_period_months ?? "—"}</TableCell>
                    <TableCell>{row.lead_time_days}</TableCell>
                    <TableCell>{row.is_active ? "Da" : "Ne"}</TableCell>
                    <TableCell align="right">
                      <IconButton
                        size="small"
                        aria-label="izmeni"
        ...
        >
          <DialogTitle>
            {editingId != null ? "Izmena vrste obaveze" : "Nova vrsta obaveze"}
          </DialogTitle>
          <DialogContent>
            <TextField
              margin="dense"
            ...
            <FormControl fullWidth margin="dense">
              ...
              <Select
                ...
                onChange={(e) =>
                  this.setState({
                    subject_kind: e.target.value as ProcessType["subject_kind"],
                  })
                }
              >
            ...
            <TextField
              ...
              onChange={(e) =>
                this.setState({ default_period_months: e.target.value })
              }
            />
            <TextField
              margin="dense"
              ...
              onChange={(e) =>
                this.setState({ lead_time_days: e.target.value })
              }
            />
            <FormControl fullWidth margin="dense">
              ...
              <Select
                ...
                label="Aktivan"
                onChange={(e) =>
                  this.setState({ is_active: e.target.value === "1" })
                }
              >
