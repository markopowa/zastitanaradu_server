    ...
    if (error || !item) {
      return (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <Alert severity="error">{error ?? "Klijent nije pronađen."}</Alert>
          <Button
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <Paper sx={{ overflow: "auto" }}>
          <Table size="small">
            ...
            <TableBody>
              {bindings.filter((b) => b.is_active).length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} align="center">
                ...
                bindings
                  ...
                  .map((b) => (
                    <TableRow key={b.id}>
                      <TableCell>{b.process_type_name}</TableCell>
                      <TableCell>
                        {b.employee
                      ...
                      <TableCell>{formatDate(b.next_run_at)}</TableCell>
                    </TableRow>
                  ))
              )}
            </TableBody>
            ...
            <TableBody>
              {runs.length === 0 ? (
                ...
                runs.slice(0, 20).map((r) => (
                  <TableRow key={r.id}>
                    <TableCell>{r.process_type_name}</TableCell>
                    <TableCell>{r.subject_snapshot?.name ?? "—"}</TableCell>
