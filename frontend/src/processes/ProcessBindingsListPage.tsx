...
class ProcessBindingsListPageInner extends Component<Props, State> {
  ...
  load = (): void => {
    ...
    if (client_company_id) params.client_company_id = Number(client_company_id);
          ...
          <Paper sx={{ overflow: "auto" }}>
            <Table size="small">
              ...
              <TableBody>
                {items.map((row) => (
                  <TableRow key={row.id}>
                    <TableCell>{row.process_type_name}</TableCell>
                    <TableCell>{subjectLabel(row)}</TableCell>
                    <TableCell>{row.next_run_at ?? "—"}</TableCell>
        ...
        >
          ...
          <DialogContent>
            <FormControl fullWidth margin="dense">
              ...
              <Select
                ...
                label="Vrsta obaveze"
                onChange={(e) =>
                  this.setState({ new_process_type: e.target.value })
                }
              >
                {types.map((t) => (
                  <MenuItem key={t.id} value={String(t.id)}>
            ...
            <FormControl fullWidth margin="dense">
              ...
              <Select
                ...
                label="Subjekt"
                onChange={(e) =>
                  this.setState({ new_subject_kind: e.target.value })
                }
              >
            ...
            {new_subject_kind === "EMPLOYEE" && (
              <FormControl fullWidth margin="dense">
                ...
                <Select
                  ...
                  label="Zaposleni"
                  onChange={(e) =>
                    this.setState({ new_employee: e.target.value })
                  }
                >
                  {employees.map((e) => (
            ...
            {new_subject_kind === "EQUIPMENT" && (
              <FormControl fullWidth margin="dense">
                ...
                <Select
                  ...
                  label="Oprema"
                  onChange={(e) =>
                    this.setState({ new_equipment: e.target.value })
                  }
                >
                  {equipment.map((e) => (
                    <MenuItem key={e.id} value={String(e.id)}>
            ...
            {new_subject_kind === "CLIENT_COMPANY" && (
              <FormControl fullWidth margin="dense">
                ...
                <Select
                  ...
                  label="Klijent"
                  onChange={(e) =>
                    this.setState({ new_client_company: e.target.value })
                  }
                >
                  {clients.map((c) => (
                    <MenuItem key={c.id} value={String(c.id)}>
            ...
            <TextField
              ...
              fullWidth
              value={new_period}
              onChange={(e) => this.setState({ new_period: e.target.value })}
            />
            <TextField
              margin="dense"
              ...
              onChange={(e) =>
                this.setState({ new_next_run_at: e.target.value })
              }
          ...
          <DialogActions>
            ...
            <Button
              ...
              disabled={
                !new_process_type ||
                (new_subject_kind === "EMPLOYEE" && !new_employee) ||
                (new_subject_kind === "EQUIPMENT" && !new_equipment) ||
                (new_subject_kind === "CLIENT_COMPANY" && !new_client_company)
              }
            >
              Dodaj
            </Button>
