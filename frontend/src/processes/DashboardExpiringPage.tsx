        ...
        >
          <FormControl size="small" sx={{ minWidth: 100 }}>
            ...
            <Select
              ...
              label="Dana"
              onChange={(e) =>
                this.handleFilterChange("days", Number(e.target.value))
              }
            >
          ...
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select
              ...
              label="Klijent"
              onChange={(e) =>
                this.handleFilterChange("client_company_id", e.target.value)
              }
            >
          ...
          <FormControl size="small" sx={{ minWidth: 160 }}>
            ...
            <Select
              ...
              label="Subjekt"
              onChange={(e) =>
                this.handleFilterChange("subject_kind", e.target.value)
              }
            >
              <MenuItem value="">Svi</MenuItem>
          ...
          <FormControl size="small" sx={{ minWidth: 200 }}>
            ...
            <Select
              ...
              label="Vrsta obaveze"
              onChange={(e) =>
                this.handleFilterChange("process_type_id", e.target.value)
              }
            >
              <MenuItem value="">Svi</MenuItem>
              {processTypes.map((t) => (
                ...
                {items.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={4} align="center">
                      Nema zapisa koji ističu u izabranom periodu.
                    </TableCell>
                  </TableRow>
                ) : (
                  items.map((run) => (
                    <TableRow key={run.id}>
                      <TableCell>
                        {run.subject_snapshot?.name ??
                          run.subject_snapshot?.kind ??
                          "—"}
                      </TableCell>
                      <TableCell>{run.process_type_name}</TableCell>
                      <TableCell>{formatDate(run.valid_until)}</TableCell>
