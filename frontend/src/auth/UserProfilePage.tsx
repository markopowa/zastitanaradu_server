...
class UserProfilePage extends Component<Props, State> {
    ...
    handleProfileSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
        ...
        if (!username?.trim()) {
            this.setState({ profileMessage: { type: "error", text: "Korisničko ime je obavezno." } });
            return;
        }
        ...
        try {
            ...
            this.setState((prev) => ({
                ...prev,
                profileMessage: { type: "success", text: "Profil je uspešno sačuvan." },
            }));
        } catch (err) {
            const text =
    ...
    handleChange =
        (field: keyof Pick<State, "currentPassword" | "newPassword" | "confirmPassword">) =>
        (e: ChangeEvent<HTMLInputElement>) => {
            this.setState((prev) => ({ ...prev, [field]: e.target.value, message: null }));
        };
    ...
    handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
        ...
        if (!newPassword || newPassword.length < 8) {
            this.setState({ message: { type: "error", text: "Nova lozinka mora imati najmanje 8 karaktera." } });
            return;
        }
        if (newPassword !== confirmPassword) {
            this.setState({ message: { type: "error", text: "Nova lozinka i potvrda se ne poklapaju." } });
            return;
        }
        ...
        try {
            ...
            this.setState({
                ...
                newPassword: "",
                confirmPassword: "",
                message: { type: "success", text: "Lozinka je uspešno promenjena." },
            });
        } catch (err: unknown) {
            const detail =
            ...
            <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
                <Paper sx={{ p: 3, maxWidth: 600, mb: 3, width: "100%" }}>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        Korisničko ime, ime i prezime možete slobodno menjati. Email nije moguće menjati.
                    </Typography>
                        ...
                        <Stack spacing={2} sx={{ maxWidth: 400 }}>
                            ...
                            {profileMessage && (
                                <Typography
                                    variant="body2"
                                    sx={{
                                        color: profileMessage.type === "error" ? "error.main" : "success.main",
                                    }}
                                >
                                    {profileMessage.text}
                                </Typography>
                ...
                <Paper sx={{ p: 3, maxWidth: 600, width: "100%" }}>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        Unesite trenutnu lozinku i novu lozinku (min. 8 karaktera).
                    </Typography>
...
const mapDispatchToProps = (dispatch: AppDispatch): DispatchProps => ({
    dispatchUpdateProfile: (payload) => dispatch(updateProfile(payload)).unwrap(),
});
