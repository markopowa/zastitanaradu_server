...
class UserProfilePage extends Component<Props, State> {
  ...
  handleProfileSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (!username?.trim()) {
      this.setState({
        profileMessage: { type: "error", text: "Korisničko ime je obavezno." },
      });
      return;
    }
    ...
    try {
      ...
      const detail =
        (err as { response?: { data?: { detail?: string } } })?.response?.data
          ?.detail ?? "Greška pri promeni lozinke.";
      ...
      >
        <Paper sx={{ p: 3, maxWidth: 600, mb: 3, width: "100%" }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Korisničko ime, ime i prezime možete slobodno menjati. Email nije
            moguće menjati.
          </Typography>
