    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue({
                message: "Neuspešno učitavanje profila",
        ...
        try {
            ...
            return rejectWithValue(
                (error as { response?: { data?: { detail?: string } } })
                    ?.response?.data?.detail ?? "Greška pri kreiranju role",
            );
        }
    },
);
...
export const updateRole = createAsyncThunk(
    ...
    async (
        ...
        try {
            ...
            return rejectWithValue(
                (error as { response?: { data?: { detail?: string } } })
                    ?.response?.data?.detail ?? "Greška pri izmeni role",
            );
        }
    },
);
...
export const deleteRole = createAsyncThunk(
    ...
    async (id: number, { rejectWithValue, dispatch }) => {
        try {
            ...
            return rejectWithValue(
                (error as { response?: { data?: { detail?: string } } })
                    ?.response?.data?.detail ?? "Greška pri brisanju role",
            );
        }
    },
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(login.rejected, (state, action) => {
                state.loading = false;
                state.error =
                    (action.payload as string) ?? "Greška pri prijavi";
            })
            ...
            .addCase(loadMe.rejected, (state, action) => {
                ...
                const payload = action.payload as
                    ...
                    | string
                    | undefined;
                const status = typeof payload === "object" && payload?.status;
                if (status === 401) {
                    state.isAuthenticated = false;
                    state.user = undefined;
                }
            ...
            .addCase(updateProfile.rejected, (state, action) => {
                state.error =
                    (action.payload as string) ?? "Greška pri izmeni profila";
            })
