...
export const login = createAsyncThunk(
    ...
    async (payload: LoginPayload, { rejectWithValue }) => {
        try {
            const result = await tokenManager.login(payload.username, payload.password);
            ...
            const msg =
                typeof detail === "string"
                    ...
                    : (Array.isArray(detail)
                          ...
                          : JSON.stringify(err?.response?.data ?? "Greška pri izmeni profila"));
...
export const loadUsers = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await api.get<AuthUser[] | { results: AuthUser[] }>("/auth/users/");
...
export const loadRoles = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            const response = await api.get<Role[] | { results: Role[] }>("/auth/roles/");
            const data = response.data;
            return Array.isArray(data) ? data : (data?.results ?? []);
        } catch (error: unknown) {
...
export const createRole = createAsyncThunk(
    "auth/createRole",
    async (payload: { name: string; permissions: number[] }, { rejectWithValue, dispatch }) => {
        try {
...
export const updateRole = createAsyncThunk(
    "auth/updateRole",
    async (
        { id, name, permissions }: { id: number; name: string; permissions: number[] },
        ...
        try {
            ...
            const msg =
                typeof err?.response?.data?.detail === "string"
                    ...
                    : JSON.stringify(err?.response?.data ?? "Greška pri kreiranju korisnika");
            return rejectWithValue(msg);
        }
    },
);
    ...
    async (
        ...
        try {
            ...
            const msg =
                typeof err?.response?.data?.detail === "string"
                    ? err.response.data.detail
                    : JSON.stringify(err?.response?.data ?? "Greška pri izmeni korisnika");
            return rejectWithValue(msg);
        }
    },
);
...
export const { logoutCompleted, setAuthUser, clearAdminError } = authSlice.actions;
