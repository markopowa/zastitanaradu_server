...
function PermissionGateComponent({ permissions, permission, anyOf, children }: Props): React.ReactNode {
    if (permission != null && !hasPermission(permissions, permission)) return null;
    if (anyOf != null && anyOf.length > 0 && !hasAnyPermission(permissions, anyOf)) return null;
    return children;
}
