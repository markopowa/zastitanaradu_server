...
const CLIENT_FIELDS: TemplateField[] = [
  ...
  { key: "client.email", label: "Email firme", snippet: "{{ client.email }}" },
  {
    key: "client.website",
        ...
        >
          <FormControl size="small" sx={{ minWidth: 220 }}>
            ...
            <Select
              ...
              onChange={(e) =>
                this.setState({
                  filter_context_type: e.target.value as ContextType,
                })
              }
            >
                    ...
                    <TableRow key={tpl.id}>
                      ...
                      <TableCell>
                        {CONTEXT_OPTIONS.find(
                          (c) => c.value === tpl.context_type,
                        )?.label ?? tpl.context_type}
                      </TableCell>
                      <TableCell>{tpl.category?.name ?? "—"}</TableCell>
                      ...
                      <TableCell align="right">
                        ...
                        <PermissionGate permission="documents.delete_documenttemplate">
                          <Button
                            ...
                            onClick={() => this.confirmDelete(tpl.id)}
                          >
                            Obriši
                          </Button>
                        </PermissionGate>
          ...
          <DialogContent>
            ...
            <FormControl fullWidth margin="dense">
              ...
              <Select
                ...
                required
              >
                {CONTEXT_OPTIONS.filter((c) => c.value !== "").map((opt) => (
            ...
            {editingId == null ? (
                ...
                {create_mode === "FROM_DOCUMENT" ? (
                  <FormControl fullWidth margin="dense">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        this.setState({
                          document_file_id: e.target.value as string,
                        })
                      }
                    >
                      <MenuItem value="">—</MenuItem>
                      {this.state.documents.map((d: DocumentFile) => (
                    ...
                    >
                      Fajl se otprema na ekranu “Dokumenti”. Ovde ga samo biramo
                      kao izvor za šablon.
                    </Typography>
                  </FormControl>
                ) : (
                  <Box sx={{ mt: 2 }}>
                    ...
                    {upload_file && (
                      ...
                      >
                        Izabrani fajl: {upload_file.name}
                      </Typography>
                    )}
                  </Box>
                )}
              ...
              <>
                <TextField
                  ...
                  onChange={(e) =>
                    this.setState({ template_body: e.target.value })
                  }
                />
                <Box sx={{ mt: 1.5 }}>
                  <Typography variant="subtitle2" gutterBottom>
                    Polja iz sistema (klikni da ubaciš u tekst)
                  </Typography>
                  <Box
                    sx={{
                      display: "flex",
                  ...
                  >
                    {availableFields.map((f) => (
                      <Chip
                        ...
                        variant="outlined"
                        onClick={() => {
                          const textarea = this.templateBodyRef.current;
                          const current = this.state.template_body ?? "";
                          if (!textarea) {
                            this.setState({
                              template_body:
                                (current ? current + " " : "") + f.snippet,
                            });
                            return;
                          }
                          const start =
                          ...
                          this.setState({ template_body: next }, () => {
                            const pos = start + f.snippet.length;
                            textarea.focus();
                  ...
                  <Box sx={{ mt: 2 }}>
                    ...
                    <List dense>
                      {Array.from(
                        new Set(
                          Array.from(
                            template_body.matchAll(/\{\{\s*([^}]+?)\s*\}\}/g),
                          ).map((m) => m[1].trim()),
                        ),
                      ).map((ph) => (
                        <ListItem key={ph} sx={{ py: 0.25 }}>
                          <ListItemText primary={`{{ ${ph} }}`} />
                        </ListItem>
                      ))}
                    </List>
                    <Typography variant="caption" color="text.secondary">
                      Ovo je samo pregled placeholder-a u tekstu. Mapiranje na
                      polja iz sistema rešavaš kroz sam Jinja2 izraz (npr.
                      &#123;&#123; employee.first_name &#125;&#125;).
                    </Typography>
                  </Box>
                )}
